...
CREATE DATABASE IF NOT EXISTS `FilRouge` DEFAULT CHARACTER SET utf8 ;
...
CREATE TABLE ProgressionsCommande(
   IdProgressionCommande INT AUTO_INCREMENT PRIMARY KEY,
   IdCommande INT,
   IdEtatCommande INT,
...
ALTER TABLE
	Rubriques
ADD
	CONSTRAINT Fk_Rubriques_Rubriques_1 FOREIGN KEY(IdRubrique_1) REFERENCES Rubriques(IdRubrique);
...
ALTER TABLE
	Utilisateurs
ADD
	CONSTRAINT Fk_Utilsateurs_Roles FOREIGN KEY(IdRole) REFERENCES Roles(IdRole);
...
ALTER TABLE
	Commandes
ADD
	CONSTRAINT Fk_Commandes_Utilisateurs FOREIGN KEY(IdUtilisateur) REFERENCES Clients(IdUtilisateur),
ADD
...
ADD
	CONSTRAINT Fk_Approvisionnements_Fournsisseurs FOREIGN KEY(IdFournisseur) REFERENCES Fournisseurs(IdFournisseur); 
